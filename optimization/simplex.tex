...
\iitem{1.}\emph{Primal Problem}: Introduce \emph{primal slack( variable)s}
$x_{n+i}=b_i-\sum_{j=1}^na_{ij}x_j\geq 0$ ($1\leq i\leq m)$\smallskip
\iitem{2.}\emph{Dual Problem}: Introduce \emph{dual slack( variable)s}
...
\iiitem{(a)}\emph{Basic (dependent) variables}: ${\cal B}_{\rm primal}=\{
x_{P(n+i)}\}_{1\leq i\leq m}$, ${\cal B}_{\rm dual}=\{ y_{P(j)}\}_{1\leq j\leq
n}$\smallskip
\iiitem{(b)}\emph{Nonbasic (independent) variables}: ${\cal N}_{\rm primal}=\{
...
\iiitem{(e)}A teableau is \emph{optimal} iff primal- \'and dual feasible
\smallskip
...
\iitem{1.}If $\forall 1\leq j\leq n,\bar{c}_j\leq 0$, stop (tableau is optimal)
\smallskip
...
\iiitem{(a)}E.g., apply \emph{largest coefficient rule}: select any $k\in
...
\iitem{1.}If $\forall 1\leq i\leq m,\bar{b}_i\geq 0$, stop (tableau is optimal)
\smallskip
\iitem{2.}Select as a \emph{leaving variable} any $x_{P(n+l)}$ s.t. $l\in
