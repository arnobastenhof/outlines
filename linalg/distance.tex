...
\iitem{2.}\emph{Root mean square (RMS) deviation} $\rms{a-b}$ between
$a,b\in\R^m$: $\|a-b\|/\sqrt{m}$\smallskip
\iitem{3.}\emph{Examples}:
\iiitem{(a)}\emph{Feature distance}: If $a,b$ are feature vectors,
then the feature distance $\dist{a}{b}$ measures their dissimilarity.\smallskip
\iiitem{(b)}\emph{Document dissimilarity}: If $a,b$ are histograms of word
occurrences for two documents, then $\dist{a}{b}$ is called the document
dissimilarity.\smallskip
\iiitem{(c)}\emph{RMS prediction error}: When $y,\widehat{y}$ are
...
\iitem{4.}For measuring dissimilarity, $\dist{a}{b}$ implicitly
