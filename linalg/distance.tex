...
\itemitem{1.}\emph{Properties}:
\itemitemitem{(a)}\emph{Non-negativity}: $\dist{a}{b}\geq 0$\smallskip
\itemitemitem{(b)}\emph{Identity of indiscernables}: $\dist{a}{b}\leftrightarrow
a=b$\smallskip
\itemitemitem{(c)}\emph{Symmetry}: $\dist{a}{b}=\dist{b}{a}$\smallskip
\itemitemitem{(d)}\emph{Triangle inequality/sub-additivity}: $\dist{a}{c}\leq
\dist{a}{b}+\dist{b}{c}$\smallskip
\itemitem{2.}\emph{Root mean square (RMS) deviation} $\rms{a-b}$ between
$a,b\in\R^m$: $\|a-b\|/\sqrt{m}$\smallskip
\itemitem{3.}\emph{Examples}:
\itemitemitem{(a)}\emph{Feature distance}: If $a,b$ are feature vectors,
then the feature distance $\dist{a}{b}$ measures their dissimilarity.\smallskip
\itemitemitem{(b)}\emph{Document dissimilarity}: If $a,b$ are histograms of word
occurrences for two documents, then $\dist{a}{b}$ is called the document
dissimilarity.\smallskip
\itemitemitem{(c)}\emph{RMS prediction error}: When $y,\widehat{y}$ are
...
\itemitem{4.}For measuring dissimilarity, $\dist{a}{b}$ implicitly
...
\itemitem{1.}\emph{Properties}:\smallskip
\itemitemitem{(a)}$\mu_{\alpha x}=\alpha\mu_x$ and $\sigma_{\alpha x}=|\alpha|
\sigma_x$\smallskip
\itemitemitem{(b)}$\mu_{x+\alpha 1}=\mu_x+\alpha$ (implying $\mu_{x-\mu_x1}=0$)
and $\sigma_{x+\alpha 1}=\sigma_x$\smallskip
\itemitem{2.}$\mu_x^2+\sigma_x^2=\mu_x^2+{1\over m}(\|x\|^2-2x^T(\mu_x1)+
\|\mu_x1\|^2)=2\mu_x^2+{\|x\|^2\over m}-2\mu_x{x^T1\over m}={\rm rms}(x)^2$
\smallskip
\itemitem{3.}\emph{Standardization} $z_i={x_i-\mu_x\over\sigma_x}$ of
$x\in\R^m$, indicating how many std's $x_i$ lies above $\mu_x$\smallskip
\itemitemitem{(a)}$\mu_z={1\over\sigma_x}\avg{x-\mu_x1}=0$ (by B1c)\smallskip
\itemitemitem{(b)}$\sigma_z={1\over\sigma_x}\std{x-\mu_x1}=1$ (by B1a,b)
\smallskip

\item{C.}\emph{Correlation coefficient} $\rho_{a,b}$ of $a,b\in\R^m$,
measuring how the entries $a,b$ vary together\smallskip
\itemitem{1.}\emph{Definition}, for $\widetilde{a}=a-\mu_a1$, $\widetilde{b}=
...
\itemitemitem{(a)} Defined iff $\sigma_a\not=0$ and $\sigma_b\not=0$ (else no
variation), noting
$\sigma_a=0\leftrightarrow\|\widetilde{a}\|=0\leftrightarrow\widetilde{a}=0
\leftrightarrow a=\mu_a1$\smallskip
\itemitem{2.}\emph{Extreme values}: uncorrelated ($0$) or perfect positive- or
...
\itemitem{3.} $\rho_{a,b}$ re-expressed in terms of \emph{standardization}:
...
\itemitem{1.}$\Sigma(i,j)={1\over m}\sum_{k=1}^m(a_{ki}-\mu_{a_i})(a_{kj}-
\mu_{a_j})$\smallskip
\itemitem{2.}$\Sigma(i,j)=\sigma_{a_i}\sigma_{a_j}\rho_{a_i,a_j}$ if
$\sigma_{a_i},\sigma_{a_j}\not=0$\smallskip
\itemitemitem{(a)}In particular, $\Sigma(i,i)=\sigma_{a_i}^2$\smallskip
\itemitemitem{(b)}$\bigwedge_{i\not=j}a_i,a_j$ uncorrelated $\leftrightarrow$
