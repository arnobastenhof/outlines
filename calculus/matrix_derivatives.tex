...
\iitem{1.} Scope: multi-valued functions of multiple variables\medskip
...
\iitem{2.} Notations
\iiitem{(a)}\emph{Matrix calculus} collects the partial derivatives
...
\iiitem{(b)} Physics favors \emph{tensor index notation} (Ricci calculus or
Einstein summation)\smallskip
% ; two variants:
% \iiitemitem{II.} Einstein summation
...
\iitem{1.}Scalar- or vector-values variables\medskip
...
\smallskip\iiitem{(a)}\emph{Tangent matrix} ${\partial Y\over\partial x}=
\left({\partial y_{ij}\over\partial x}\right)_{ij}\in\R^{m\times n}$

\iiitem{(b)}\emph{Gradient matrix} ${\partial y\over\partial X}=
